    ...
    \item Consider some excited state population, $N_2$. $e^{-\Gamma_Ht}$
        ...
        \begin{figure}[H]
            ...
            \begin{tikzpicture}
                ...
                \draw[<->] (6.3,1) -- (7.7,1) node[anchor=south west] {FWHM $= \Gamma_H$};
            \end{tikzpicture}
        \end{figure}
    ...
    \item Broadening arises due to Maxwell-Boltzmann distribution of velocities,
        ...
        \begin{figure}[H]
            ...
            \begin{tikzpicture}
                ...
                \draw[-Latex] (10,1.4) node[anchor=west] {FWHM $= \Delta\om_I$} -- (9.6,1.4);
...
\section{The Fabry-Perot Etalon}
...
\subsection{Path difference between successive rays}
\vspace{-20pt}
\begin{align}
    P.D. &= AB + BD = AB + BC = AC \\
         &= 2d\cos\theta \\
...
\begin{align}
    E_T &= E_0t^2\left(1+r^2e^{i\delta} + r^4e^{2i\delta} + r^6e^{3i\delta}+\dots\right) \text{ - Geometric Progression}\\
        &= \sum_0^\infty r^{2n}e^{in\delta} = \frac{1}{1-r^2e^{i\delta}} = \frac{E_0T}{1-Re^{i\delta}},\; T = |t|^2,\; R = |r|^2
\end{align}
...
\begin{example}
    ...
    We can use this etalon to revolve to 1 part in $10^8$.
\end{example}
...
The electric field distribution needs to reproduce after a round trip, so it needs to be its own Fourier transform, i.e. Gauss-Hermite modes.
\begin{align}
...
\begin{enumerate}
    ...
    \item Optical Diode: Consists of 2 elements that rotate the polarisation of the light.
        ...
        So the laser is forced to lase in a single direction; eliminates spatial hole burning and makes it easier to achieve single frequency oscillation.
\end{enumerate}
...
\begin{itemize}
    \item Active Mode Locking
        \begin{itemize}
            \item Electro-Optic Modulator: used with a polariser, applying a voltage can let light through at repetition rate, by rotating polarisation.
                \begin{figure}[H]
                    \centering
                    \begin{tikzpicture}
                        \filldraw[fill=lightgray,draw=black] (0,0) -- (-0.5,0) -- (-0.5,1.5) -- (0,1.5) arc (150:210:1.5cm);
