...
\begin{titlepage}
    ...
    \HRule \\[0.7cm]
    {\huge\bfseries Quantum Optics}\\[0.4cm]
    \HRule \\[1.5cm]
    ...
    \begin{minipage}{0.4\textwidth}
        \begin{flushright} \large
            \emph{Lecturer:} \\ Prof. Viv Kendon
        \end{flushright}
    \end{minipage}\\[2cm]
    \vfill
    \includegraphics[width=0.7\textwidth]{qo.jpg}\\[1cm]
    \vfill
\end{titlepage}
\tableofcontents
...
\begin{itemize}
    ...
    \item definition of the creation and annihilation operators, commutation relations, and the adjoint and self-adjoint operators
...
\begin{itemize}
    \item Explain and state the description of the electromagnetic field in terms of modes and polarization
    \item Be familiar with the equivalence between a mode of the field and a quantum harmonic oscillator
    \item To explain the form of (but not derive) expressions for the Hamiltonian of the electromagnetic field, and the electric and magnetic fields in terms of the creation and annihilation operators
...
\begin{align}
    \E(\vr,t) &= \frac{\p}{\p t}\unl{A},\quad \B(\vr,t) = \del\times\unl{A}(\vr,t) \\
...
\begin{align}
    H &= \frac12\e_0 \int \left(\E\cdot\E + c^2\B\cdot\B\right)\,dV = 2\e_0 V \sum_{\unl{k},s} \om_k^2 A_{\unl{k},s}A_{\unl{k},s}^* \\
...
\chapter{Quantum States of the Electromagnetic Field.}
\section{Learning Outcomes}
...
\chapter{Towards the Classical Limit}
...
\begin{align}
    \hat{X}_1 &= \frac12\left(\ha+\hag\right)\to \hq,\quad \hat{X}_2 = \frac{1}{2i}\left(\ha-\hag\right)\to \hp,\quad [\hat{X}_1,\hat{X}_2] = \frac{i}{2} \\
    \langle\xo\rangle_\alpha &= \langle\alpha|\xo|\alpha\rangle = \frac12 \left\{\langle\alpha|\ha|\alpha\rangle + \langle\alpha|\hag|\alpha\rangle\right\} = \frac12 \left\{\alpha+\alpha^*\right\} = \text{Re}(\alpha) \\
...
\begin{align}
    \hh_{int} &= i\hbar\left(\eta^*\ha^2e^{i(\om_p - 2\om)t} - h.c.\right) = i\hbar\left(\eta^*\ha^2 - \eta(\hag)^2\right),~ \om_p = 2\om \\
...
\begin{itemize}
    ...
    \item Be familiar with the Pauli operators as quantum description of the two-level atom.
...
\begin{align}
    |\psi_1,n\rangle &\equiv |e\rangle|n\rangle = |i\rangle,\quad |\psi_2,n\rangle \equiv |g\rangle|n+1\rangle = |f\rangle \\
...
\chapter{Rotating Wave Approximation}
...
\begin{figure}[H]
    \centering
    \begin{tikzpicture}
        \draw[-Latex] (0,0) -- (0,3.4) node[anchor=east] {$P_i(t)$};
        \draw[-Latex] (0,0) -- (4.5,0) node[anchor=north] {$\lambda t$};
        \draw[teal] (0,3) node[anchor=east,black] {1} cos (0.4,1.5) sin (0.8,0) cos (1.2,1.5) sin (1.6,3) cos (2,1.5) sin (2.4,0) cos (2.8,1.5) sin (3.2,3) node[anchor=west,black] {$n=5$} cos (3.6,1.5) sin (4,0);
        \draw (1,0.05) -- (1,-0.05) node[anchor=north] {1};
        \draw (2.5,0.05) -- (2.5,-0.05) node[anchor=north] {2};
        \draw (3.9,0.05) -- (3.9,-0.05) node[anchor=north] {3};

        \draw[-Latex] (6,0) -- (6,3.4) node[anchor=east] {$P_i(t)$};
        \draw[-Latex] (6,0) -- (10.5,0) node[anchor=north] {$\lambda t$};
        \draw[purple] (6,3) node[anchor=east,black] {1} cos (6.1,1.6) sin (6.2,0.3) cos (6.3,1.4) sin (6.4,1.8) cos (6.5,1.55) sin (6.6,1.3) cos (6.75,1.45) sin (7,1.5) -- (7.5,1.5) cos (7.55,1.4) sin (7.6,1.3) cos (7.65,1.5) sin (7.7,1.7) cos (7.75,1.5) sin (7.8,1.25) cos (7.85,1.5) sin (7.9,1.75) cos (7.95,1.5) sin (8,1.2) cos (8.05,1.5) sin (8.1,1.9) cos (8.15,1.5) sin (8.2,1) cos (8.25,1.5) sin (8.3,2.1) cos (8.35,1.5) sin (8.4,0.9) cos (8.45,1.5) sin (8.5,2.2) cos (8.55,1.5) sin (8.6,0.8) cos (8.65,1.5) sin (8.7,2.1) cos (8.75,1.5) sin (8.8,0.9) cos (8.85,1.5) sin (8.9,2) cos (8.95,1.5) sin (9,1.1) cos (9.05,1.5) sin (9.1,1.8) cos (9.15,1.5) sin (9.2,1.3) cos (9.25,1.4) sin (9.3,1.5) cos (9.4,1.4) sin (9.45,1.3) cos (9.5,1.5) sin (9.6,1.8) cos (9.7,1.5) sin (9.8,1.2) cos (9.9,1.5) sin (10,1.8) cos (10.1,1.6) node[anchor=south west,black] {$\langle n\rangle=5$} sin (10.2,1.4);
        \draw (6.8,0.05) -- (6.8,-0.05) node[anchor=north] {5};
        \draw (7.6,0.05) -- (7.6,-0.05) node[anchor=north] {10};
        \draw (8.4,0.05) -- (8.4,-0.05) node[anchor=north] {15};
        \draw (9.2,0.05) -- (9.2,-0.05) node[anchor=north] {20};
        \draw (10,0.05) -- (10,-0.05) node[anchor=north] {25};
    \end{tikzpicture}
\end{figure}
...
\begin{align}
    |i\rangle &= |e\rangle|n\rangle,\quad |f\rangle = |g\rangle|n+1\rangle \\
    \hh(n) &= \hbar\begin{pmatrix}n\om+\frac12\om_0 & \lambda\sqrt{n+1} \\ \lambda\sqrt{n+1} & (n+1)\om-\frac12\om_0 \end{pmatrix}
\end{align}
...
\begin{figure}[H]
    ...
    \end{tikzpicture}
\end{figure}
The phenomenon shown above is known as vacuum Rabi splitting, where we get anharmonic energy levels due to coupling as $\lambda\neq0$.
...
\chapter{Photodetection and Correlation Functions}
...
Now what about the field at the detector?
\begin{align}
...
\chapter{Quantum Theory of Beamsplitters}
\section{Learning Outcomes}
We will develop the quantum mechanical theory of the beamsplitter.
After studying this section, you should
\begin{itemize}
    \item Be familiar with the classical description of a lossless beamsplitter.
...
\chapter{Interference and Coherence}
\section{Learning Outcomes}
We will discuss interference, and both the classical and quantum definitions of first and second order coherence.
After studying this section, you should
\begin{itemize}
    \item Be able to calculate the output state of an interferometer with number states or coherent states in the input modes.
...
\chapter{Beam Splitters and Interferometry}
...
\chapter{Polarisation in Two-Mode States}
\section{Learning Outcomes}
...
\begin{itemize}
    ...
    \item Understand the action of a linear polariser in terms of projective measurements; be familiar with the use of the projection operator, and be able to discuss the state before and after the measurement.
\end{itemize}
...
\end{align}
Hence, for the amplitudes, $A_x,A_y\to A_\theta,A_{\theta'}$:
\begin{align}
...
\chapter{Density Matrices and Quantum States}
\section{Learning Outcomes}
...
\begin{figure}[H]
    \centering
    \begin{tikzpicture}
        \draw[-Latex] (0,0) node[anchor=south] {1} -- (1,0);
        \draw[-Latex] (1,0) -- (3,0);
        \draw (3,0) -- (4,0) node[anchor=south] {3,  Bob};
        \draw[-Latex] (2,2) node[anchor=west] {0} -- (2,1);
        \draw[-Latex] (2,1) -- (2,-1);
        \draw (2,-1) -- (2,-2) node[anchor=east] {Alice,  2};
        \draw[dashed] (0.75,-1.25) rectangle (3.25,1.25);
        \draw (0.75,1.25) node[anchor=south east] {Charlie} -- (3.25,-1.25);
    \end{tikzpicture}
    \vspace{-20pt}
\end{figure}
...
\begin{figure}[H]
    \centering
    \begin{tikzpicture}
        \draw[-Latex] (0,0) node[anchor=south] {$|3\rangle$} -- (1,0);
        \draw[-Latex] (1,0) -- (3,0);
        \draw (3,0) -- (4,0) node[anchor=south] {V};
        \draw[-Latex] (2,2) node[anchor=west] {$0\rangle$} -- (2,1);
        \draw[-Latex] (2,1) -- (2,-1);
        \draw (2,-1) -- (2,-2) node[anchor=east] {H};
        \draw[dashed] (0.75,-1.25) rectangle (3.25,1.25);
        \draw (0.75,1.25) -- (3.25,-1.25);
    \end{tikzpicture}
\end{figure}
